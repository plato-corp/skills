...
티로 작업에서는 현재 워크스페이스 지침·승인된 소스 레지스트리·검증된 프로젝트 인수인계에서 디자인 시스템과 온톨로지의 정본 위치를 찾는다. 다른 브랜드 작업에는 해당 브랜드의 승인된 시스템을 적용한다. 비공개 저장소 이름·문서 경로·검토 번호·수치·이미지는 공개 스킬에 복제하지 않는다.

- 색상·다크모드·로고를 변경하면 해당 사용 가이드와 모드별 로고 계약을 읽는다.
- 티로 타이틀 서체를 쓰면 한/영/일 섞어짜기 계약을 읽고 모든 환경에서 광학 보정을 필수 적용한다. 혼식·영문 단독 역할을 구분하며 본문·버튼·wordmark는 기존 계약을 유지한다.
- 그래픽을 만들면 현행 스타일 정본·구현 규칙·승인된 참고 이미지 목록을 읽는다. 구도와 서사는 매체에 맞추되 확정 스타일을 임의로 재탐색하지 않는다.
- 그래픽 생성이 작업 범위에 있고 정본에서 참고 이미지 입력을 이미 허용했다면, 실제 이미지를 열어 생성 도구에 첨부한다. 같은 허락을 다시 요구하지 않는다. 원본·파생 자산 기록과 결과 검수를 따르며 로고·정확한 글자·UI는 실제 소재와 조판으로 합성한다. 생성 허용은 외부 게시 권한과 구분한다.

실제 읽은 ref·커밋·파일을 프로젝트의 승인된 기록에 남긴다. 검토 브랜치가 지정돼 있으면 현재 상태와 SHA를 확인하고, 병합 여부와 현행 정본 지정에 따라 조회한다. 접근 실패나 미병합 종료를 반영 완료로 판단하거나 값을 추정하지 않는다. 규칙 확정과 Figma·제품 전수 적용 상태를 구분한다.

프로젝트 평가에 해당 타이틀 보정, 색·로고 역할, 그래픽 정본 일치와 참고 이미지 입력 증거를 연결한다. 아래 일반적인 폰트 선택·레퍼런스 탐색 지침은 승인된 티로 계약을 덮어쓰지 않는다.
